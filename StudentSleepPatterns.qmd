...
<<<<<<< HEAD
Figure 1: Bar Graph of Average Sleep Duration by University Year \# Load ggplot2 package library(ggplot2)

# Join the averages with the confidence intervals to add the confidence interval values

averages_with_ci \<- averages %\>% left_join(conf_intervals, by = "University_Year")

# Now plot with the confidence intervals

# Bar plot for Sleep Duration

ggplot(averages_with_ci, aes(x = University_Year, y = avg_sleep_duration)) + geom_bar(stat = "identity", fill = "skyblue", width = 0.7) + geom_errorbar(aes( ymin = avg_sleep_duration - (sleep_duration_ci\[\[1\]\]\[2\] - sleep_duration_ci\[\[1\]\]\[1\]) / 2, ymax = avg_sleep_duration + (sleep_duration_ci\[\[1\]\]\[2\] - sleep_duration_ci\[\[1\]\]\[1\]) / 2 ), width = 0.25) + labs(title = "Average Sleep Duration by University Year", y = "Average Sleep Duration (hours)", x = "University Year") + theme_minimal()
...
Figure 2: Bar Graph of Average Physical Activity Level by University Year \# Bar plot for Physical Activity ggplot(averages_with_ci, aes(x = University_Year, y = avg_physical_activity)) + geom_bar(stat = "identity", fill = "lightgreen", width = 0.7) + geom_errorbar(aes( ymin = avg_physical_activity - (physical_activity_ci\[\[1\]\]\[2\] - physical_activity_ci\[\[1\]\]\[1\]) / 2, ymax = avg_physical_activity + (physical_activity_ci\[\[1\]\]\[2\] - physical_activity_ci\[\[1\]\]\[1\]) / 2 ), width = 0.25) + labs(title = "Average Physical Activity by University Year", y = "Average Physical Activity (units)", x = "University Year") + theme_minimal()
...
# Plot code for sleep duration by university year
...
# Plot code for physical activity by university year
averages_with_ci <- averages %>%
  left_join(conf_intervals, by = "University_Year")
ggplot(averages_with_ci, aes(x = University_Year, y = avg_physical_activity)) +
  ...
  theme_minimal()
```
>>>>>>> e9f59b9c30c14e9be293b1115c0ee45f20179914
